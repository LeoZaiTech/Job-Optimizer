...
export async function importJobsFromUrls(inputs) {
  ...
  results.forEach((result) => {
    ...
    sources.push({
      importedCount: result.jobs.length,
      input: result.input,
      sourceType: result.sourceType
    });
  });
...
function normalizeFetchError(error, url) {
  if (error && typeof error === "object" && (error.name === "AbortError" || error.name === "TimeoutError")) {
    return new Error(`Fetch timed out for ${url}`);
  }

  return error instanceof Error ? error : new Error(`Fetch failed for ${url}`);
}
...
function isRemoteText(value) {
  ...
  if (hasRemoteExclusion) {
    return false;
  }

  if (hasRemoteOptionSignal) {
    return true;
  }
